...
import org.xbib.elasticsearch.jdbc.client.transport.BulkTransportClient;
import org.xbib.elasticsearch.jdbc.cron.CronExpression;
import org.xbib.elasticsearch.jdbc.cron.CronThreadPoolExecutor;
...
public class JDBCFeeder {
     ...
     * ClusterStateResponse with StatesMetadata
     */
    static {
        MetaData.registerFactory(StateMetaData.TYPE, StateMetaData.FACTORY);
    }
    ...
    /**
     * This ingest is the client for reading/writing the flow states
     */
    ...
    public JDBCFeeder readFrom(Reader reader) {
        ...
        try {
            ...
            // before running, create the flow
            createFlow(map, settings);
        } catch (IOException e) {
    ...
    protected Flow createFlow(Map<String, Object> spec, Settings settings) throws IOException {
        String strategy = XContentMapValues.nodeStringValue(spec.get("strategy"), "simple");
        this.flow = StrategyLoader.newFlow(strategy);
        logger.debug("strategy {}: flow class {}, spec = {} settings = {}",
                strategy, flow.getClass().getName(), spec, settings.getAsMap());
        this.ingestFactory = createIngestFactory(settings);
        // out private ingest, needed for having a client in the flow
    ...
    public JDBCFeeder start() throws Exception {
        ...
        this.feederThread = new Thread(new Feeder(flow, definitions));
